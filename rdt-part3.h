...
int time_out(int fd, int seq, char *msg, int *msgLen, int &rSeq, struct timeval &tv = tv1) {
...
/* same as time_out()
used by the rdt_close part
*/
int time_out2(int fd, int seq, char *msg, int *msgLen) {
        ...
        if (rv == -1) {
            ...
            if (FD_ISSET(fd, &readfds)) {
                ...
                if (r_ack == 0) {
                    ...
                    if (r_seq == seq) {
                        printf("time_out: Received in order ACK, seq#: %d\n", r_seq);
                        rv = 1;
                    } else {
                        printf("time_out: Received out of order ACK, seq#: %d\n", r_seq);
                        rv = 2;
                    }
                }
            } else {
...
int rdt_close(int fd) {
    ...
    int temp, rSeq;
    // The rdt_close phase uses TWAIT, which is longer than TIMEOUT.
    // If more data are coming in, than reACK with revSeq-1, and reset timer.

    struct timeval tv = tv2;
    while (1) {
        temp = time_out(fd, revSeq-1, NULL, NULL, rSeq, tv);
        if (temp == 5 || temp == 3 || temp == 4) {
            tv = tv2; // reset timer
            printf("rdt_close: received retransmission, send out dup ACK\n");
            udt_send(fd, ack_pkt, pktLen, 0);
        } else if (temp == 0) { // timeout occurs
            break;
        }
    }
